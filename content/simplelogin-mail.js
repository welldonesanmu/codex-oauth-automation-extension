...
async function waitForAliasChange(previousAlias = '', previousClipboardAlias = '', timeoutMs = 30000) {
  const start = Date.now();
  let lastSnapshot = '';
  ...
  while (Date.now() - start < timeoutMs) {
    ...
    if (clipboardAlias && clipboardAlias !== previousAlias && clipboardAlias !== previousClipboardAlias) {
    ...
    const aliases = collectSimpleLoginAliases();
    const nextAlias = aliases.find((alias) => !previousAlias || alias !== previousAlias) || '';
    const snapshotKey = aliases.join('|');
    if (nextAlias) {
      debugLog('SimpleLogin alias changed', { previousAlias, nextAlias, elapsedMs: Date.now() - start, aliases });
      return nextAlias;
    }
...
async function fetchSimpleLoginAlias(payload = {}) {
  ...
  const currentAlias = readSimpleLoginAlias();
  ...
  const deletedAlias = await deleteExistingSimpleLoginAliasBeforeGenerate();
  if (deletedAlias) {
    debugLog('SimpleLogin deleted alias before generate', { deletedAlias });
  }
  ...
  if (shouldClickRandomAlias) {
    ...
    setSimpleLoginGenerationState({
      deletedAlias: stateAfterDelete?.deletedAlias || deletedAlias || '',
      deleteCompleted: Boolean(stateAfterDelete?.deleteCompleted || deletedAlias),
  ...
  const nextAlias = await waitForAliasChange(currentAlias, clipboardAliasBeforeClick);
  clearSimpleLoginGenerationState();
