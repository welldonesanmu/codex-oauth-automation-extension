...
if (!window.__MULTIPAGE_ADDY_LISTENER_READY__) {
  ...
  chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    ...
    if (inFlightRequest?.key === requestKey && inFlightRequest.promise) {
    ...
    resetStopState();
    const promise = fetchAddyAlias(message.payload);
...
async function waitForAddyButtonStable(button, options = {}) {
  const { timeoutMs = 12000, stableMs = 1200 } = options;
...
async function waitForConfirmCreateAliasButton(entryButton, timeoutMs = 15000) {
  const start = Date.now();
  while (Date.now() - start < timeoutMs) {
    throwIfStopped();
...
async function waitForAddySuccessCopyState(timeoutMs = 20000) {
  const start = Date.now();
  while (Date.now() - start < timeoutMs) {
    throwIfStopped();
...
async function waitForAddyClipboardAlias(expectedAlias = '', previousAlias = '', timeoutMs = 5000) {
  const start = Date.now();
...
async function waitForAliasChange(previousEmail = '', timeoutMs = 30000) {
  const start = Date.now();
  while (Date.now() - start < timeoutMs) {
    throwIfStopped();
...
async function fetchAddyAlias(payload = {}) {
  ...
  const confirmButton = await waitForConfirmCreateAliasButton(entryButton, 15000);
